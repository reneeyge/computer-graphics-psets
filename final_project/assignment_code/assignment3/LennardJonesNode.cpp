...
namespace GLOO {
 ...
 : SceneNode() {
    ...
    for (size_t i = 0; i < state_.positions.size(); i++){
        ...
        for (int j = i + 1; j < state_.positions.size(); j++) {
            float spring_const = 2*0.1 * (6 / 0.1) * (6 / 0.1);
            float rest_length = glm::length(state_.positions[i] - state_.positions[j]);
            system_.AddSpring(spring_const, rest_length, i, j);
        }
    }
