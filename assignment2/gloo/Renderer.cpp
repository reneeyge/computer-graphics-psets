...
void Renderer::RecursiveRetrieve(const SceneNode& node, RenderingInfo& info, const glm::mat4& model_matrix) const { // recursive dfs storing local to world matrices
    
    if (node.IsActive()) {
        glm::mat4 curr_model_matrix = model_matrix * node.GetTransform().GetLocalToParentMatrix(); // get matrix of current level
        RenderingComponent* ptr = node.GetComponentPtr<RenderingComponent>(); // get rendering component of node
        if (ptr != nullptr) { // not at root
            info.emplace_back(ptr, curr_model_matrix);
        }
        for (int i = 0; i < node.GetChildrenCount(); i++) {
            RecursiveRetrieve(node.GetChild(i), info, curr_model_matrix); // go thru every child from this level
        }
    }
}
