...
void PhongShader::SetTargetNode(const SceneNode& node,
  ...
  // take care of the cases where some of textures is nullptr.
    ...
    const auto& ambient_texture = material_ptr->GetAmbientTexture();
    if (ambient_texture != nullptr) {
        (*ambient_texture).BindToUnit(0);
        SetUniform("ambient_texture", 0);
    }
    const auto& diffuse_texture = material_ptr->GetDiffuseTexture();
    if (diffuse_texture != nullptr) {
        (*diffuse_texture).BindToUnit(1);
        SetUniform("diffuse_texture", 1);
    }
    const auto& specular_texture = material_ptr->GetSpecularTexture();
